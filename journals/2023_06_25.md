...
- redis的AOF是什么 #小林coding #card
  ...
  :END:
	...
	- Redis 是***先执行写操作命令后，才将该命令记录到 AOF 日志里的***
		- 不是[[WAL]]类型的日志，所以有可能丢失数据
		- AOF写操作由**主进程**执行，有可能阻塞下一个命令
	- AOF日志刷盘策略
	...
	- 虚拟地址vs物理地址
		...
		- 虚拟内存[[多级页表]] #card
		  id:: 6498257a-8f72-4b25-8183-ae0b0d41e8ed
			- 32 位单级页表，在页大小 `4KiB`($2^{12}$) 的环境下，一个进程的页表需要装下`100 多万个`($2^{20}$)「页表项」，并且每个页表项是占用 4 字节大小的，于是相当于每个页表需占用 4MB 大小的空间。
			- 32位二级页，一级页表覆盖到了全部虚拟地址空间，二级页表在需要时创建。一级页面占用1024*4字节=`4KiB`
	...
	- Linux 的[[虚拟地址]]空间是如何分布的？#card
	  id:: 649833be-4e46-4c4f-a156-d9342f11ade4
		...
		- 以64位linux系统为例
			...
			- **文件映射段**(memory map region)，包括**动态库、共享内存**等，从低地址开始向上增长（64位linux如此，32位相反，[跟硬件和内核版本有关](http://lishiwen4.github.io/linux/linux-process-memory-location)）；`mmap()`
...
- 如何准备项目经历 #面试 #项目经验 #猎头vivi #富途
	- 如何做到高效有条理的描述?
		...
		- 对项目的业务逻辑和技术架构有充分准备
			...
			- 涉及的技术充分掌握
			- [[疑难杂症的案例]]
- 今天用了logseq，学习效率非常棒，太趁手的兵器了。需要记忆的知识点，直接同步到anki，再也没有担心遗忘的负担了。输入->笔记->记忆，流程非常丝滑。 #学习方法