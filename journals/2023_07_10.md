- 睡眠时间短，会导致快速眼动睡眠被压缩，会影响睾酮分泌。而睾酮会影响自信，骨密度，红细胞数等。如果只睡4 5个小时，快速眼动睡眠会被压缩得非常短。【提高身体素质｜医学博士兔叭咯的科普是对是错??？运动学博士的专业解析-哔哩哔哩】【视频标记点 04:46】 https://b23.tv/bMlE0wP #睡眠 #深度睡眠 和 #快速眼动睡眠 都非常重要 ![2023-07-13-02-58-51.jpeg](../assets/2023-07-13-02-58-51.jpeg)
- 今天整理一下这一周的[[短期目标]]
...
- 如何对map的value进行排序 #富途面试题
	- ```Go
	  ...
	  ```
- [[隔离性]]：[[MVCC]]解决隔离读问题，2PL/OCC解决写冲突的问题 #innodb的隔离性实现 #终于把分布式事务讲明白了 #card
  id:: 64abcb75-3b7b-427b-912c-8b960a895135
  [[原子性]]：WAL和2PC #innodb的原子性实现 #终于把分布式事务讲明白了 
...
- 先有事务的概念，然后再补上事务之间可见性的问题 #终于把分布式事务讲明白了 #card
  id:: 64abcde3-2332-4713-85f5-5893125bf1e9
	- Jim Gray于1981年VLDB描述了事务的原子性、一致性和持久性，在此基础上，Haerder和Reuter在1983年中提出了**事务的[[隔离性]]**并提出术语“ACID”，自此，事务的ACID四个性质成为业内标准术语。
...
- 原子性和持久化流程： #innodb的原子性实现 #innodb的持久化实现
...
- undo日志属于逻辑日志，redo是物理日志 #card #《MySQL技术内幕》
  id:: 64abfef7-f1ab-4f02-82eb-5b86fe837c34
	...
	- r物理日志：因为​​mysql​​数据最终是保存在数据页中的，物理日志记录的就是数据页变更。
-