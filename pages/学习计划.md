- 非技术
	- 漫读
	    [正念课程](https://www.notion.so/4af378c99b77448cade199f2bde84b64?pvs=21) ：晚上班车上听
	    [活出心花怒放的人生](https://www.notion.so/9fd6873d70b3405782deba2f61e94ec1?pvs=21) ：晚上班车上听
	    [腾讯音乐协会-唱歌基础班](https://www.notion.so/3814b4cd97b64a1cbea346017e1e7509?pvs=21) ：晚上班车听
	- 记忆
	    [英语语法新思维1](https://www.notion.so/1-dc9458d5ad9f4e43a24ed031f1cb0ed4?pvs=21) ：晚上用ipad，边看边做笔记
	    力量训练基础：工作番茄时间休息期间看一下
- 技术
	- [[算法和数据结构]]
	    快速浏览完《labuladong算法》，了解基本套路，记入艾宾浩斯
	    刷[LeetCode热门100题](https://leetcode.cn/studyplan/top-100-liked/)，9/100 #leetcode 
	    [剑指offer](https://leetcode.cn/studyplan/coding-interviews/)
	    [左程云200题](https://b23.tv/B5kDQrb)
	    快速浏览完《算法》，了解基本原理，找到重点难点，计入艾宾浩斯
	- 八股文
	    [小林coding](https://xiaolincoding.com/) 包含[[操作系统]]，[[网络]]，[[redis]]，[[mysql]]
	    [路人张的面试笔记](https://www.mianshi.online/) 跟小林coding类似，有[[系统设计]]题，智力题
	    **[马士兵教育](https://www.bilibili.com/video/BV14P411M7yR)** 看一下[[mq]]
		- [[系统设计]]题：
		    [路人张的面试笔记](https://www.mianshi.online/1283.html)
		    [常见的架构设计面试题 | Beritra](https://www.notion.so/Beritra-6c85ee99a5894ac0a6c4a9dc8e2aeb4b?pvs=21) 
		    [系统设计八股文-分布式ID生成器](https://www.notion.so/ID-56f99e774dc9417ca5fa337940b3b8da?pvs=21) 
		    [系统设计入门](https://github.com/donnemartin/system-design-primer/blob/master/README-zh-Hans.md#%E7%B3%BB%E7%BB%9F%E8%AE%BE%E8%AE%A1%E5%85%A5%E9%97%A8)
	- 语言
	    [**go实现**](https://draveness.me/golang/)
	    **[Go 程序员面试笔试宝典](https://golang.design/go-questions)**
	- [[数据库]]
		- [[mysql]]
			- [图解MySQL](https://xiaolincoding.com/mysql/) [[图解MySQL]] #小林coding
			    MVCC
			    存储结构
			    隔离级别
			    索引优化
			    锁特性
			    集群，主备同步
			- [[mysql 45讲]]
			- **[[MySQL技术内幕]]** 正在阅读
			- 数据库系统实现
		- [[redis]]
			- [redis数据结构](https://xiaolincoding.com/redis/)
			    redis常用数据结构和命令
			    看完小林code的redis数据结构
			- Redis实现：各种版本集群的实现 [Redis面试八股文第一弹](https://zhuanlan.zhihu.com/p/457798648)
		- [[rocksdb]]实现
		- [[tikv]]实现
	- 交易系统
	    看内网的交易系统细节，和数据一致性问题
	    wx支付可靠事件中心的实现原理和源码
	    微信支付中的微服务模式（三）----反脆弱性
	- 技术爱好
		- 分布式数据库
			- DDIA
			- 弱一致性论文
		- 机器学习
		    [《动手学深度学习》](https://zh.d2l.ai/index.html)