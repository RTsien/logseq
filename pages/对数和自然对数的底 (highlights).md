title:: 对数和自然对数的底 (highlights)
author:: [[云风的 BLOG]]
full-title:: "对数和自然对数的底"
category:: #articles
url:: https://blog.codingnow.com/2025/04/log_e.html
summary:: 最近读了一本书：《数学的雨伞下》。阅读体验非常好，这本书用浅显的语言，科普了许多深刻的道理。这本书所介绍的知识结构比较类似我挺喜欢的另一本《从一到无穷大》，但讲解更为细致一些，以至于如果事先明白这些知识，甚至会觉得有些冗长。但细细品味，会觉得理解能更深一层。
...
把对数用于快速计算乘法，选用怎样的底并不重要。当等差数列的...
![](https://blog.codingnow.com/favicon.ico)

- Highlights first synced by [[Readwise]] [[Apr 15th, 2025]]
	- 制作对数表的直接原因当然是为了简化大数乘除法的计算。对数概念的提出在幂概念建立之前，而现在的数学教学中，一般却是从幂自然推导到对数的。似乎后者才是自然而然的。这应该是因为，古人研究数学，最初是为了解决现实中的问题。所以，乘法必须有对应的几何意义。比如，计算正方形面积需要把计算边长的平方；立方体的体积需要计算边长的立方。更高次的幂却难有对应的几何意义，有理数幂则更为抽象。 ([View Highlight](https://read.readwise.io/read/01jrtnr7za2ckc7et5td74yd36))
	- 人无法以上帝视角在宇宙空间中做测量，只能以地球为基点。所以，天文尺度的计算都依赖三角学。把天文（以及地理这种地球尺度的）数字问题化为三角函数，然后再加以计算。比如，测量地球到太阳的距离、地球到月球的距离、地球的直径都是这样。因为这些尺度都非常大，如果计算精度不够，就容易失之毫厘，差之千里。 ([View Highlight](https://read.readwise.io/read/01jrtnrst0qs3qr962gsckny71))