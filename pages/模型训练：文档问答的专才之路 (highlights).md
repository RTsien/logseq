...
- Highlights first synced by [[Readwise]] [[Nov 29th, 2023]]
	- 常用的检索方法包括传统的倒排索引检索和基于[[embedding]]的语义检索。传统倒排索引检索通常采用分词和停用词过滤，然后构建相应的倒排索引。通过基于token粒度的匹配召回，倒排索引检索系统具有速度快、精度高的优势。但由于严格匹配的逻辑，其无法捕获语义上的相似性，泛化能力较差。 ([View Highlight](https://read.readwise.io/read/01hgdp8tgx33rzwsshkbnt7jzz))
